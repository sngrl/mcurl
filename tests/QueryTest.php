...
class QueryTest extends CaseTest
{
    ...
    {
        $i = 0;
        $urls = array($this->url('/simple-0.txt'), $this->url('/simple-1.txt'), $this->url('/simple-2.txt'), $this->url('/simple-3.txt'));
        foreach ($urls as $url) {
            $this->req->add(array(CURLOPT_URL => $url));
        }
    ...
    public function testSpider()
    {
        ...
        $this->req->add(array(CURLOPT_URL => $this->url('/simple-1.txt')));
        $this->req->add(array(CURLOPT_URL => $this->url('/simple-2.txt')));
        while ($this->req->run() || $this->req->has()) {
            while ($this->req->has()) {
                ...
                for ($i = 0; $i < pow($num, 2); $i++) {
                    $this->req->add(array(CURLOPT_URL => $this->url('/simple-' . ($num + 1) . '.txt')));
                }
                $this->assertEquals(200, $result->getHttpCode());
                $this->assertNotEmpty($result->getBody());
            }
